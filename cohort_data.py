...
def sort_by_cohort(filename):
    ...
    for line in the_file:
        ...
        if line[-1] == "Fall 2015":
            fall_15.append(line[0:2])
            # "".join(fall_15)

        elif line[-1] == "Spring 2016":
            spring_16.append(line[0:2])

        elif line[-1] == "Winter 2016":
            winter_16.append(line[0:2])

        elif line[-1] == "Summer 2016":
            summer_16.append(line[0:2])

        elif line[-1] == "G":
            ghosts.append(line[0:2])

# plan: to write function of list comprehension (below) to joing first and last names
# for each cohort

    x = [" ".join(item) for item in fall_15]

# also to try: append all cohorts to all_students list, and then try the join
